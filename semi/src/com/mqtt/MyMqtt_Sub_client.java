...
//2. MqttCallback 인터페이스의 메소드를 오버라이딩
...
public class MyMqtt_Sub_client implements MqttCallback{
	...
	public MyMqtt_Sub_client init(String server, String clientId) {
		...
		try {
			//broker에 subscribe하기 위한 클라이언트 객체를 생성
			mqttclient = new MqttClient(server,  clientId);
			//클라이언트 객체에 callback을 셋팅 - subscribe하면서 적절한 시점에 메소드가 호출될 수 있다.
	...
	//커넥션이 종료 되었을때 호출 - 통신오류로 연결이 끊어진 경우에 호출된다!
	@Override
	...
	//메시지가 도착하면 호출되는 메소드 - topic(broker에 구독신청한 topic명), MqttMessage는 메시지
	@Override
	...
	public boolean subscribe(String topic) {
		try {
		if(topic!=null) {
				mqttclient.subscribe(topic, 0); //Qos는 메시지를 전달하고 관리하는 방법
			}
		} catch (MqttException e) {
			e.printStackTrace();
			return false;
	...
	public static void main(String[] args) {
		...
		//MqttClient객체가 broker에 연결되고 구독신청까지 완료됨.
